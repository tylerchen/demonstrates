...
public class InternalResourceView extends AbstractUrlBasedView {
	...
	public void setExposedContextBeanNames(String[] exposedContextBeanNames) {
		this.exposedContextBeanNames = new HashSet<String>(Arrays
				.asList(exposedContextBeanNames));
	}
	...
	protected String prepareForRendering(HttpServletRequest request,
		...
		if (this.preventDispatchLoop) {
			...
			if (path.startsWith("/") ? uri.equals(path) : uri
					.equals(StringUtils.applyRelativePath(uri, path))) {
				throw new ServletException(
						"Circular view path ["
								+ path
								+ "]: would dispatch back "
	...
	protected boolean useInclude(HttpServletRequest request,
		...
		return (this.alwaysInclude || WebUtils.isIncludeRequest(request) || response
				.isCommitted());
	}
